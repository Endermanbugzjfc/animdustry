...
in {
  ...
  web = with pkgs; mkShellNoCC {
    ...
    in shellHook + "\n" + ''
      ...
      # The emscripten cache shipped in the Nix store is read-only;
      # emcc needs a writable copy to populate ports and system libs.
      export EM_CACHE="''${XDG_CACHE_HOME:-$HOME/.cache}/emscripten/${emscripten.version}"
      if [ ! -d "$EM_CACHE" ]; then
        mkdir -p "$EM_CACHE"
        cp -r --no-preserve=mode,ownership ${emscripten}/share/emscripten/cache/. "$EM_CACHE" 2>/dev/null || true
      fi
    '';
  };
}
