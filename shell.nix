let
  ...
  packages = [
    ...
    pkgs.nimble
    (pkgs.callPackage "${nnl}/package.nix" {})
...
in {
  ...
  win = let
    ...
    pkgs-cross-lock = pkgs-lock.pkgsCross.mingwW64;

    cross = with pkgs-cross-lock.buildPackages; [
      gcc
      nim
    ];
  ...
  in { wineprefix ? "~/.local/share/wineprefixes/animdustry" }: pkgs.mkShellNoCC {
